...
#%matplotlib inline
import pandas_datareader,datetime,re,math,operator 
...
class StockOfToday:
    ...
    #returns sharpe ratio of individual stocks
    def sharpe_ratio(self):
        num_ports=15000
        np.random.seed(101)
        stocks = self.return_stock_df()[['Adj Close']]
        log_return = np.log(stocks/stocks.shift(1))
        all_weights = np.zeros((num_ports,1))
        ret_arr = np.zeros(num_ports)
        vol_arr = np.zeros(num_ports)
        sharpe_arr = np.zeros(num_ports)

        for i in range(num_ports):
            weights = np.array(np.random.random(1))
...
class stock_menu_looper():
    ...
    def __init__(self, alloc):
        ...
        list_of_stocks = input('Enter individual or comma sporated list of Stock tickers (ex: tsla-2,aapl,AMZN...): ')
        my_stocks = re.split(', |,', list_of_stocks) #check stock share input format
        my_stock_objects = []
        ...
        if(self.alloc==0):
            for i in my_stocks:
                if '''-''' in i:
                    tic = i.split('-')[0]
                    shar = i.split('-')[1]
                    my_stock_objects.append(StockOfToday(tic,start,end,shares=int(shar)))
                else:
                    my_stock_objects.append(StockOfToday(i,start,end))
        ...
        #new Stock Of Today list object with no (allo)$ allocated to portfolio
            ...
            for i in my_stocks:
                if '''-''' in i:
                    ...
                    my_stock_objects.append(StockOfToday(i.upper(),start,end))
...
def portfolio_menu():
    ...
    print (30 * "-" , "Stock-List Menu" , 30 * "-")
    print("""
        1. Sharpe Ratio and $ to Allocation
        
        2. Company Finantials
        
        3. Full info (IN TESTING)
    ...
    #3
        ...
        allo = input('How much to allocate ($) ?')
        try:
            df = stock_menu_looper(allo).init_stock_obj().portfolio_div_growth_sharpe_df()
            return df.loc[:, (df != 0).any(axis=0)]
...
class stock_of_today_list():
    ...
    #
    def return_ticker_cumel_dict(self):
        d = {}
        for i in self.stock_list:
            d.update(i.cumel_dict())
        return d
    #
    def sharpe_ratio_looper(self):
        d = {}
        for i in self.stock_list:
            d.update({i.return_stock_ticker():float(round(i.sharpe_ratio(),3))})
        return d
    
    
    #FULL DF
    def portfolio_div_growth_sharpe_df(self):
        
        sharpe_dict = self.sharpe_ratio_looper()
        sharpe_df = pd.DataFrame.from_dict(sharpe_dict,orient='index',columns=['Sharpe'])

        cumel_df = self.return_ticker_cumel_dict()
        shares_ticker_df = pd.DataFrame.from_dict(cumel_df,orient='index',columns=['Cumel_Return'])
        
        df = pd.concat([self.return_div_df(), shares_ticker_df, sharpe_df],axis=1)
        
        #saving the DataFrame as a CSV file 
        #csv_data = df.to_csv('my_stocks_df.csv', index = True) 
        #print('\nCSV String:\n', csv_data) 
        #
        
        desired_cols = ['last adj close','div %/Y','#shares','equity','div-equity-year','Cumel_Return','Sharpe']
